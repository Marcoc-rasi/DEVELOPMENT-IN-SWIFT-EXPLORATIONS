...
func block(x: Int, y: Int, width: Int, height: Int, color: Color){
    for rawNumber in 0 ..< height {
        hline(x: x, y: y + rawNumber, length: width, color: color)
    }
}

block(x: 0, y: 0, width: 7, height: 6, color: .red)
block(x: 10, y: 7, width: 2, height: 8, color: .cyan)
/*:
 ### Fill Speed
...
func block1(x: Int, y: Int, width: Int, height: Int, color: Color){
    
    for rawNumber in 0 ..< height {
        for columnNumber in 0 ..< width {
            pixelsArray.append(Pixel(x: x + rawNumber, y: y + columnNumber, color:color))
        }
    }
}
block1(x: 10, y: 15, width: 20, height: 20, color: .yellow)
display.batchSetPixels(pixelsArray)
