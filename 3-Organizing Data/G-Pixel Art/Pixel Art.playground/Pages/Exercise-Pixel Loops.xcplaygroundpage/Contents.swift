...
func hPixel(pixelPositionStartX: Int,pixelPositionFinishX: Int,pixelPositionY: Int,color: Color){
    for i in pixelPositionStartX ... pixelPositionFinishX {
        display.setPixel(x: i, y: pixelPositionY, color: color)
    }
}

func vPixel(pixelPositionStartY: Int,pixelPositionFinishY: Int,pixelPositionX: Int,color: Color){
    for i in pixelPositionStartY ... pixelPositionFinishY {
        display.setPixel(x: pixelPositionX, y: i, color: color)
    }
}

let displaySize: Int = 19
...
func runDisplay(displaySize: Int,initPosition: Int){
    for rawPosition in initPosition ... displaySize{
        hPixel(pixelPositionStartX: initPosition, pixelPositionFinishX: displaySize,pixelPositionY: rawPosition,color: arrayColors[Int.random(in: 0..<arrayColors.count-1)])
    }
    /*for columnPosition in initPosition ... displaySize{
        vPixel(pixelPositionStartY: initPosition, pixelPositionFinishY: displaySize, pixelPositionX: columnPosition, color: arrayColors[Int.random(in: 0..<arrayColors.count-1)])
    }*/
}
