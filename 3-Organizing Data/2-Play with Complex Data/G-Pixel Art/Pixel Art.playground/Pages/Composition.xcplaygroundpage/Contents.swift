...
func legs() {
    display.batchSetPixels(block(x: 10, y: 14, width: 2, height: 4, color: .blue))
}
func torso() {
    display.batchSetPixels(block(x: 9, y: 18, width: 4, height: 3, color: .white))
}
func head() {
    display.batchSetPixels(block(x: 10, y: 21, width: 2, height: 2, color: .yellow))
}
